...
def run(input_root, simulation_period):
    wn_dict = amf_obs_time_series(BASE_AMF_DICT,
                                   input_root,
                                   complete_days_only=True,
                                   return_low_err=True)

    wn_path = os.path.join(input_root, 'ameriflux_sites')  # OK
    etrm_extract = os.path.join(wn_path, 'AMF_extracts')
    output_root = os.path.join(wn_path, 'AMF_ETRM_output')
    combo_root = os.path.join(wn_path, 'AMF_results_combo')
...
if __name__ == '__main__':
    ir = os.path.join('G:', 'WGEW', 'ETRM_Inputs')
    sim = datetime(2000, 1, 1), datetime(2000, 1, 2)
