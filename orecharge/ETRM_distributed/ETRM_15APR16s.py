...
start, end = datetime.datetime(2000, 1, 1), datetime.datetime(2001, 12, 31)
...
de = np.array(de_open.GetRasterBand(1).ReadAsArray(), dtype=float)
# de = de[480:520, 940:980]
de = np.where(np.isnan(de) == True, np.zeros(taw.shape), de)
de_open = []

raster = 'dr_4_18_2_49'
dr_open = gdal.Open('{a}\\{b}.tif'.format(a=path, b=raster))
dr = np.array(dr_open.GetRasterBand(1).ReadAsArray(), dtype=float)
# dr = dr[480:520, 940:980]
dr = np.where(np.isnan(dr) == True, np.zeros(taw.shape), dr)
dr_open = []
...
for dday in rrule.rrule(rrule.DAILY, dtstart=start, until=end):
    ...
    drew_1 = np.minimum((pDrew + ro + (1 - fcov) * (evap - (rain + mlt))), rew)
...
# Create cumulative rasters to show net over entire run
    ...
    ref_et += etrs
    et = et + evap + transp
    et = np.where(np.isnan(et) == True, 0.0, et)
...
for element in outputs:
    ...
    filename = 'C:\\Recharge_GIS\\Array_Results\\{a}_{b}.tif'.format(a=name, b=tag)
