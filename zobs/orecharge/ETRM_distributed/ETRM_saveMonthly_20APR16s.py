...
pEt = np.zeros(taw.shape)
pPrecip = np.zeros(taw.shape)
pRo = np.zeros(taw.shape)
p_delta_s_mo = np.zeros(taw.shape)
pInfil = np.zeros(taw.shape)
pEtrs = np.zeros(taw.shape)
...
for dday in rrule.rrule(rrule.DAILY, dtstart=start, until=end):
    ...
    if dday == start:
        ...
        pDe = de
        pDrew = drew
        delta_deps = taw + tew + rew - (drew + de + dr)
        # dr = pDr
        # de = pDe
        # drew = pDrew
    ...
    ks_ref = np.where(((taw - pDr) / (0.6 * taw)) < np.zeros(taw.shape), np.zeros(taw.shape), ((taw - pDr) / (0.6 * taw)))
    ...
    et = et + evap + transp
    et_ind = et / ref_et
    et = np.where(np.isnan(et) == True, et_ind, et)
    ...
    if dday.day == mo_date[1]:
            ...
            delta_deps = taw + tew + rew - (drew + de + dr)
            delta_s_mo = delta_deps - p_delta_s_mo

            outputs = [infil_mo, et_mo, precip_mo, runoff_mo, snow_ras_mo, delta_s_mo, dr]
            output_names = ['infil', 'et', 'precip', 'runoff', 'snow_ras', 'delta_s_mo', 'dr']
