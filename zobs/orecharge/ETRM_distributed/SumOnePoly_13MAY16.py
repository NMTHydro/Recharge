...
for root, dirs, files in os.walk(".", topdown=False):
    for element in files:
        if element.endswith('.shp'):
            shp_name = '{}\\{}'.format(input_folder, element)
            ...
            dataset = ppt_open
            temp = '{}\\{}_temp{}.tif'.format(temp_folder, element[20:-4], input_folder.index(input_folder))
            ...
            for feat in lyr:
                pos = input_folder.index(input_folder)
                if pos == 0:
            ...
            np.savetxt('{}\\{}.csv'.format(output_folder, name),
                       recs, fmt=['%s', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f',
                       '%1.3f', '%1.3f'], delimiter=',')

            fmt = ",".join(["%s"] + ["%10.6e"] * (recs.shape[1]-1))

            with open('{}\\{}.csv'.format(output_folder, name), 'wb') as f:
            ...
            np.savetxt('{}\\{}.csv'.format(yr_output_folder, name),
                       yr_recs, fmt=['%s', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f', '%1.3f',
                       '%1.3f', '%1.3f'], delimiter=',')

            fmt = ",".join(["%s"] + ["%10.6e"] * (recs.shape[1]-1))

            with open('{}\\{}.csv'.format(yr_output_folder, name), 'wb') as f:
