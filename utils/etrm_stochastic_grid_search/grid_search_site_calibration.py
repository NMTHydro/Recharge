...
from utils.etrm_stochastic_grid_search.site_rzsm_error_calc import calc_rzsm_error
...
outpath = '/Users/dcadol/Desktop/academic_docs_II/calibration_approach/mini_model_outputs_III/wjs/calibration_output'
sitename = 'wjs'
amf_name = 'US-Wjs'
...
if sitename == 'wjs':
    swhc_vals = [368, 347, 141, 369, 205]
    swhc_method = ['ecomodel_v1', 'ecomodel_v2', 'max_depletion_mod', 'max_depletion', 'NRCS']

elif sitename == 'seg':
    swhc_vals = [376, 319, 136, 532, 281]
    swhc_method = ['ecomodel_v1', 'ecomodel_v2', 'max_depletion_mod', 'max_depletion', 'NRCS']

elif sitename == 'ses':
    swhc_vals = [671, 614, 202, 579, 269]
    swhc_method = ['ecomodel_v1', 'ecomodel_v2', 'max_depletion_mod', 'max_depletion', 'NRCS']

elif sitename == 'mpj':
    swhc_vals = [768, 764, 228, 924, 115]
    swhc_method = ['ecomodel_v1', 'ecomodel_v2', 'max_depletion_mod', 'max_depletion', 'NRCS']

elif sitename == 'vcp':
    swhc_vals = [200, 200, 200, 200, 200]
    swhc_method = ['ecomodel_v1', 'ecomodel_v2', 'max_depletion_mod', 'max_depletion', 'NRCS']
...
data_path = '/Users/dcadol/Desktop/academic_docs_II/calibration_approach/mini_model_outputs_III/{}/taw_{}_dataset.csv'
...
for i in range(0, ((end_taw - begin_taw) / taw_step)):
    ...
    if rzsm:
        ...
        # get rid of the zero values # TODO - Should I do this?
        # data_df = data_df[data_df['average_vals_rzsm'] != 0]
        data_df = data_df[data_df['nrml_depth_avg_sm'] != 0]

        ## Uncomment two lines below to eliminate date range from rzsm.
        # print 'month range {}'.format(range(5, 9))
        # data_df = data_df[data_df.index.month.isin(range(5, 9))]
        timeseries = data_df.index.values

        data_df.to_csv(
            '/Users/dcadol/Desktop/academic_docs_II/calibration_approach/mini_model_outputs_III/{}/calibration_output/cumulatives/rzsm_model_df_{}.csv'.format(sitename, current_taw))
        ...
        data_df.to_csv('/Users/dcadol/Desktop/academic_docs_II/calibration_approach/mini_model_outputs_III/{}/calibration_output/cumulatives/cum_eta_model_df_{}_cum{}.csv'.format(sitename, current_taw, cum_int))
...
if not rzsm:
    ...
    # =========== Calculate the Chi Square Residuals ================
    chi_dictionary, dof_dict = get_chisquare_dict(model_dictionary=optimization_dict, parameter_lst=taw_list,
                                                  ...
                                                  name_extension=name_ext, cum_value=cum_int, sitename=sitename)
elif rzsm:
    # TODO - What is the corect observational error?
    error = 0.020
    absolute_error = calc_rzsm_error(sitename, error)
    print 'abs error for {} is {}'.format(sitename, absolute_error)
...
chimin_dict = taw_optimize_1d(parameter_lst=taw_list, chi_dict=chi_dictionary, outpath=outpath,
                              name=amf_name, num_obs=dof, name_extension=name_ext, swhc_vals=swhc_vals,
                              swhc_method=swhc_method)
